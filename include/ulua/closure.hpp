...
namespace ulua {
	...
	template<> struct type_traits<push_count> {
		ULUA_INLINE static int push(lua_State* L, push_count value) { return value.n; }
	};

	namespace detail {
		// Applies a function an pushes the result.
		//
		template<typename Ret, typename Args, typename F> ULUA_INLINE inline int apply_closure(lua_State* L, F& func) {
			return std::apply(
				[&]<typename... Tx>(Tx&&... args) ULUA_INLINE -> int {
		...
		//
		template<typename F> inline int push_closure(lua_State* L, F&& func) {
			...
			if constexpr (Traits::is_lambda && std::is_default_constructible_v<Func> && std::is_trivially_destructible_v<Func> &&
			              sizeof(Func) == sizeof(std::monostate)) {
			...
			//
				...
				return [&]<typename... Tx>(std::type_identity<std::tuple<Tx...>>) {
					return push_closure(L, [func](C& owner, Tx&&... args) -> Ret { return (owner.*func)(std::forward<Tx>(args)...); });
		...
		//
		template<auto F> inline int push_closure(lua_State* L, const_tag<F>) {
			...
			//
				...
				return [&]<typename... Tx>(std::type_identity<std::tuple<Tx...>>) {
					return stack::push(L, [](C& owner, Tx&&... args) -> Ret { return (owner.*F)(std::forward<Tx>(args)...); });
				}(std::type_identity<Args>{});
			}
		}
	};
	...
	//
	template<auto F>
		requires detail::Invocable<decltype(F)>
	struct type_traits<const_tag<F>> {
		ULUA_INLINE static int push(lua_State* L, const_tag<F>) { return detail::push_closure(L, const_tag<F>{}); }
	};
	template<typename F>
		requires(!Reference<std::decay_t<F>> && detail::Invocable<std::decay_t<F>> && !std::is_same_v<std::decay_t<F>, cfunction_t>)
	struct type_traits<F> {
		template<typename V> ULUA_INLINE static int push(lua_State* L, V&& func) {
	...
	// Overload helper.
	//
	template<typename... Tx> struct overload : private Tx... {
		using arguments = std::variant<detail::popped_vtype_t<typename detail::function_traits<Tx>::arguments>...>;

		inline constexpr overload()
			requires((std::is_default_constructible_v<Tx> && sizeof(Tx) == sizeof(std::monostate)) && ...)
		= default;
