#pragma once
// ulua/coroutine.hpp — Coroutine support.
//
// Wraps lua_newthread/lua_resume/lua_yield with typed argument passing. coroutine::create() spawns a new thread,
// resume() pushes arguments and resumes, and the static resume() overload yields from within a C function.
//
...
namespace ulua {
	...
	struct coroutine : state_view {
		...
		//
		template<typename Ref> static coroutine create(const Ref& f) {
			lua_State* L = f.state();
		...
		//
		template<typename... Tx> int resume(Tx&&... args) const {
		...
		//
		int status() const { return lua_status(this->L); }
		...
		//
		template<typename... Tx> static int resume(lua_State* L, Tx&&... args) {
			int nargs = stack::push(L, std::tie(std::forward<Tx>(args)...));
			return lua_yield(L, nargs);
		}
	};

	template<> struct type_traits<coroutine> {
		...
		ULUA_INLINE static bool check(lua_State* L, int& idx) { return stack::type_check<value_type::thread>(L, idx++); }
