#pragma once
// ulua/ffi.hpp — LuaJIT FFI integration (LuaJIT-only).
//
// Provides ffi::cdef and ffi::set_metatable for C type definition, ctype_id_cache for per-state type ID caching, and
// specialized userdata_wrapper/type_traits that store values inside cdata instead of userdata for zero-overhead FFI
// interop.
//
#include <atomic>
#include "stack.hpp"
...
namespace ulua::ffi {
	...
	inline CTypeID typeid_of(lua_State* state, const char* name) {
		constexpr uint32_t mask = (1u << CT_KW) | (1u << CT_STRUCT) | (1u << CT_ENUM) | (1u << CT_TYPEDEF);
		...
		// Resolve typedef / attrib chain to the underlying concrete type.
		if (id != 0) {
			CTState* cts = ctype_cts(state);
			while (ctype_istypedef(t->info) || ctype_isattrib(t->info)) t = ctype_child(cts, t);
			id = ctype_typeid(cts, t);
		}
		return id;
	}
...
namespace ulua {
	...
	//
	template<typename T>
		requires UserCType<std::remove_const_t<T>>
	struct userdata_wrapper<T> {
		...
		template<typename... Tx> constexpr userdata_wrapper(Tx&&... args) : storage(std::forward<Tx>(args)...) {}
	...
	// Ctype cache.
	//
	template<typename T> struct ctype_id_cache {
		...
		ULUA_COLD static CTypeID fetch_slow(lua_State* L) {
			...
			if (stack::create_metatable(L, userdata_mt_name<T>().data())) [[unlikely]] {
				...
				if constexpr (CTypeHasInlineCDef<user_traits<T>>) ffi::cdef(L, user_traits<T>::cdef).assert();
				// Get the type ID.
	...
	struct type_traits<userdata_wrapper<T>> {
		...
		ULUA_INLINE static bool check(lua_State* L, int& idx) {
			...
			if (!tviscdata(tv)) return false;
			return cdataV(tv)->ctypeid == cache::fetch(L);
		}
		ULUA_INLINE static userdata_wrapper<T>& get(lua_State* L, int& idx) {
			auto* tv = accel::ref(L, idx++);
			if (!tviscdata(tv)) type_error(L, idx - 1, userdata_name<std::remove_const_t<T>>().data());
			auto* cd = cdataV(tv);
			auto* wr = std::launder((userdata_wrapper<T>*)cdataptr(cd));

	#if ULUA_CTYPE_VALIDATED
			if (cd->ctypeid != cache::fetch(L)) type_error(L, idx - 1, userdata_name<std::remove_const_t<T>>().data());
	#endif
			return *wr;
		}
	};
	...
	struct user_type_traits<T> : emplacable_tag_t {
		...
		template<typename... Tx> ULUA_INLINE static int emplace(lua_State* L, Tx&&... args) {
			// Emplace the C type.
			//
		...
		template<typename V = T> ULUA_INLINE static int push(lua_State* L, V&& value) { return emplace(L, std::forward<V>(value)); }
		ULUA_INLINE static bool check(lua_State* L, int& idx) { return type_traits<userdata_wrapper<T>>::check(L, idx); }
