...
namespace ulua {
	...
	struct reference_base {
		...
		inline static bool check(lua_State*, int& slot) {
			slot++;
			return true;
		}
	...
	struct stack_reference : reference_base {
		...
		inline constexpr stack_reference(lua_State* L, int index) : L(L), index(stack::abs(L, index)), ownership_flag(true) {
			detail::assume_true(this->index != invalid_value);
		}
		inline constexpr stack_reference(lua_State* L, int index, weak_t) : L(L), index(stack::abs(L, index)), ownership_flag(false) {
			detail::assume_true(this->index != invalid_value);
		}

		inline constexpr stack_reference(stack_reference&& o) noexcept { swap(o); }
		inline constexpr stack_reference& operator=(stack_reference&& o) noexcept {
			swap(o);
			return *this;
		}
		...
		inline constexpr void reset() {
			if (valid()) {
				int s = release();
				if (ownership_flag && stack::is_absolute(s)) stack::checked_remove(L, s);
			}
		}
		inline constexpr void reset(detail::unchecked_t) {
			if (valid()) {
				int s = release();
				if (ownership_flag && stack::is_absolute(s)) stack::remove(L, s);
			}
		}
	...
	struct registry_reference : reference_base {
		...
		inline constexpr registry_reference& operator=(registry_reference&& o) noexcept {
			swap(o);
			return *this;
		}
		template<Reference Ref> inline registry_reference(const Ref& o) { assign(o); }
		template<Reference Ref> inline registry_reference& operator=(const Ref& o) {
			assign(o);
			return *this;
		}
		...
		template<typename Ref> inline void assign(const Ref& o) {
			reset();
			if (o.valid()) {
				o.push();
				L = o.L;
	...
	//
	template<Reference R> struct type_traits<R> : popable_tag_t {
		ULUA_INLINE inline static int push(lua_State* L, const R& ref) {
			ref.push();
			return 1;
		}
	...
	template<> struct type_traits<reg_key> : popable_tag_t {
		ULUA_INLINE inline static int push(lua_State* L, reg_key key) {
			stack::push_reg(L, key);
			return 1;
		}
		ULUA_INLINE inline static bool check(lua_State*, int& idx) {
			idx++;
			return true;
		}
	...
	//
	inline bool equals(const stack_reference& r1, const stack_reference& r2) { return stack::equals(r1.state(), r1.slot(), r2.slot()); }
	...
	//
	inline size_t length(const stack_reference& r1) { return stack::length(r1.state(), r1.slot()); }
