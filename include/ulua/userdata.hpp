...
namespace ulua {
	...
	//
	template<typename T> struct user_traits : nil_t {};
	template<typename T>
		requires requires { typename T::lua_traits; }
	...
	namespace detail {
		...
		template<typename T> struct userdata_fields {
			static constexpr std::tuple<> value = {};
		};
		template<UserdataHasFields T> struct userdata_fields<T> {
			static constexpr const auto& value = user_traits<T>::fields;
		};
		...
		template<UserdataHasMetatable T> struct userdata_metatable<T> {
			static constexpr const auto& value = user_traits<T>::metatable;
		};
	};
	...
	//
	namespace detail {
		inline std::atomic<size_t> unnamed_type_counter{0};
		...
		// Name generation.
		//
		template<typename T> struct userdata_namer {
#if ULUA_CTTI
			...
			static constexpr std::string_view metatable_name() {
				return {_metatable_name.data(), _metatable_name.data() + _metatable_name.size() - 1};
			}
#endif
		};
		template<NamedUsedType T> struct userdata_namer<T> {
	...
	// Userdata wrappers.
	//
	template<typename T> inline bool __userdata_tag = false;

	template<typename T> struct userdata_wrapper {
		...
		template<typename S> inline S* store() const { return (S*)(this + 1); }
		...
		inline void destroy() {
			...
			case userdata_storage::value:
				return std::destroy_at(store<T>());
			default:
				detail::assume_unreachable();
			}
		}
	};
	template<typename T> struct userdata_by_value : userdata_wrapper<T> {
		T value;
		template<typename... Tx> inline userdata_by_value(Tx&&... args)
			: userdata_wrapper<T>(&value, userdata_storage::value), value(std::forward<Tx>(args)...) {}
	};
	template<typename T> struct userdata_by_pointer : userdata_wrapper<T> {
		inline userdata_by_pointer(T* pointer) : userdata_wrapper<T>(pointer, userdata_storage::pointer) {}
	};

	// Implement type traits.
	//
	template<typename T> struct type_traits<userdata_wrapper<T>> {
		// No pusher.
	...
	template<typename T> struct user_type_traits : emplacable_tag_t {
		template<typename... Tx> ULUA_INLINE inline static int emplace(lua_State* L, Tx&&... args) {
		...
		template<typename V = T> ULUA_INLINE inline static int push(lua_State* L, V&& value) { return emplace(L, std::forward<V>(value)); }
		ULUA_INLINE inline static bool check(lua_State* L, int& idx) { return type_traits<userdata_wrapper<T>>::check(L, idx); }
	...
	template<typename T> struct user_type_traits<T&&> : user_type_traits<T> {
		ULUA_INLINE inline static T get(lua_State* L, int& idx) { return user_type_traits<T>::get(L, idx); }
	};
	template<typename T> struct user_type_traits<T*> {
