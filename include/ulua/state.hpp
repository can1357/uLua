#pragma once
// ulua/state.hpp — Lua state management.
//
// state_view provides a non-owning interface for table creation, library loading, script execution, and garbage
// collection. state adds RAII ownership with lua_close on destruction. Includes per-library descriptors for selective
// library loading.
//
#include "common.hpp"
#include "stack.hpp"
...
namespace ulua {
	...
	// Load result.
	//
	template<Reference Ref> struct load_result : Ref, detail::lazy_invocable<load_result<Ref>> {
		int retval;

		template<typename... Tx> explicit load_result(int retval, Tx&&... args) : Ref(std::forward<Tx>(args)...), retval(retval) {}

		inline bool is_error() const { return retval != 0; }
		inline bool is_success() const { return retval == 0; }
		inline void assert() const {
			if (is_error()) ulua::error(Ref::state(), error());
		}
		inline explicit operator bool() const { return is_success(); }
		const char* error() const {
			Ref::push();
			return stack::pop<const char*>(Ref::state());
		}

		// Decay to function result for state helpers.
		//
		inline function_result decay_to_invocation()
			requires(std::is_same_v<Ref, stack_reference>)
		{
#if ULUA_DEBUG
			if (Ref::slot() != stack::top(Ref::state())) ulua::error(Ref::state(), ">> Decay from non-top slot <<");
#endif
			Ref::release();
	...
	struct state_view {
		...
		// Creates a table.
		//
		template<typename T = table> inline T make_table(reserve_table rsvd = {}) {
		...
		// Creates a metatable.
		//
		template<typename T = table> inline std::pair<T, bool> make_metatable(const char* name) {
			bool inserted = stack::create_metatable(L, name);
			return std::pair{T{L, stack::top_t{}}, inserted};
		}

		template<typename T = stack_table> inline T get_metatable(const char* name) {
		...
		template<typename Key> inline auto operator[](Key&& key) {
			return detail::make_table_proxy<true>(L, LUA_GLOBALSINDEX, false, std::forward<Key>(key));
		}
		...
		template<typename... Tx> inline void open_libraries(Tx&&... descriptors) { (open_libraries(descriptors), ...); }

		// Parses a script and returns the chunk as a function.
		//
		template<Reference R = stack_reference> inline load_result<R> load_file(const char* path) {
			return load_result<R>{luaL_loadfile(L, path), L, stack::top_t{}};
		}
		template<Reference R = stack_reference> inline load_result<R> load(std::string_view script, const char* name = "") {
		...
		template<Reference Ref> inline function_result script_file(const char* path, const basic_environment<Ref>& env) {
			auto result = load_file<stack_reference>(path);
			if (!result) return result.decay_to_invocation();
			env.set_on(result);
			return std::move(result)();
		...
		// Collects garbage.
		//
		inline void collect_garbage() { lua_gc(L, LUA_GCCOLLECT, 0); }
	};
	struct state : state_view {
		...
		//
		inline void reset() { reset(luaL_newstate()); }
		inline void reset(lua_Alloc allocf, void* allocd) { reset(lua_newstate(allocf, allocd)); }
