#pragma once
// ulua/environment.hpp — Lua environments.
//
// basic_environment<Ref> extends basic_table with set_on() for lua_setfenv binding. this_environment is a
// pseudo-argument type that captures the caller's function environment from the callstack.
//
#include "table.hpp"
...
namespace ulua {
	...
	//
	template<Reference Ref> struct basic_environment : basic_table<Ref> {
		...
		template<Reference RefY> inline basic_environment(lua_State* L, create tag, const basic_table<RefY>& fallback) {
