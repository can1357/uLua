#pragma once
// ulua/userdata_metatable.hpp — Automatic metatable generation for userdata types.
//
// Introspects C++ types via concepts to auto-generate __index/__newindex (with field dispatch tables), __tostring,
// __gc, __eq/__lt/__le, __len, __pairs/__ipairs, and full arithmetic operators. Uses an embedded Lua dispatcher script
...
namespace ulua {
	namespace detail {
		...
		template<typename T> struct default_key_type {
			using type = size_t;
		};
		template<HasKeyType T> struct default_key_type<T> {
			using type = typename T::key_type;
		};
		template<typename T> using default_key_type_t = std::conditional_t<std::is_same_v<typename default_key_type<T>::type, std::string>,
		                                                                   const char*, typename default_key_type<T>::type>;

		template<typename T> struct default_value_type;
		template<HasValueType T>
			requires(!HasMappedType<T>)
		struct default_value_type<T> {
			using type = typename T::value_type;
		};
		template<HasMappedType T> struct default_value_type<T> {
			using type = typename T::mapped_type;
		};
		template<typename T> using default_value_type_t = typename default_value_type<T>::type;

		template<typename T>
		concept HasToString = requires(const T& v) { v.to_string(); };
		template<typename T>
		concept HasLength = requires(const T& v) { v.length(); };
		template<typename T>
		concept HasSize = requires(const T& v) { std::size(v); };
		template<typename T>
		concept EqComparable = requires(const T& a, const T& b) { a == b; };
		template<typename T>
		concept GeComparable = requires(const T& a, const T& b) { a >= b; };
		template<typename T>
		concept GtComparable = requires(const T& a, const T& b) { a > b; };
		template<typename T>
		concept LeComparable = requires(const T& a, const T& b) { a <= b; };
		template<typename T>
		concept LtComparable = requires(const T& a, const T& b) { a < b; };
		template<typename T>
		...
		template<typename T>
		concept KvIterable = requires(const T& v) {
			std::begin(v)->first;
			std::begin(v)->second;
		};
		template<typename T>
		concept FindIndexable = requires(const T& v) { v.find(std::declval<default_key_type_t<T>>())->second; };
		template<typename T>
		concept Indexable = requires(const T& v) { v[std::declval<default_key_type_t<T>>()]; };
		template<typename T>
		concept NewIndexable = requires(T& v) { v[std::declval<default_key_type_t<T>>()] = std::declval<default_value_type_t<T>>(); };
		template<typename T>
		concept Negable = requires(const T& v) { -v; };
		template<typename T, typename O>
		concept Addable = requires(const T& v, const O& v2) { v + v2; };
		template<typename T, typename O>
		concept Subable = requires(const T& v, const O& v2) { v - v2; };
		template<typename T, typename O>
		concept Mulable = requires(const T& v, const O& v2) { v * v2; };
		template<typename T, typename O>
		concept Divable = requires(const T& v, const O& v2) { v / v2; };
		template<typename T, typename O>
		concept Idivable = requires(const T& v, const O& v2) { size_t(v / v2); };
		template<typename T, typename O>
		concept Modable = requires(const T& v, const O& v2) { v % v2; };
		template<typename T, typename O>
		concept Powable = requires(const T& v, const O& v2) { pow(v, v2); };
		template<typename T, typename O>
		concept Xorable = requires(const T& v, const O& v2) { v ^ v2; };
		...
		template<typename... Tx> static void run_through(lua_State* L, const char* code, Tx&&... args) {
	...
	//
	template<typename T> struct metaproperty_descriptor {
		T value;
		meta field;

		template<typename V> inline constexpr metaproperty_descriptor(meta field, V&& value)
			: value(std::forward<V>(value)), field(field) {}
	};
	template<auto Value> static constexpr auto property(meta field) { return metaproperty_descriptor{field, constant<Value>()}; }
	template<typename T> static constexpr auto property(meta field, T&& value) {
		return metaproperty_descriptor<std::decay_t<T>>{field, std::forward<T>(value)};
	}

	struct constant_getter_tag {};
	template<typename T> struct constant_getter : constant_getter_tag {
		T value;

		template<typename V> constexpr constant_getter(V&& value) : value(std::forward<V>(value)) {}
	};
	template<typename T> constant_getter(T&&) -> constant_getter<std::decay_t<T>>;

	struct bytecode_property {
		const char* code;
	};

	template<typename G, typename S> struct member_descriptor {
		G getter;
		S setter;
		const char* name;

		template<typename GX, typename SX> inline constexpr member_descriptor(const char* name, GX&& getter, SX&& setter)
			: getter(std::forward<GX>(getter)), setter(std::forward<SX>(setter)), name(name) {}

		inline void write_getter(stack_table& tbl) const {
			if constexpr (!std::is_same_v<std::decay_t<G>, nil_t>) {
				if constexpr (std::is_base_of_v<constant_getter_tag, std::decay_t<G>>) {
					detail::run_through(tbl.state(), "return function(tbl, key, value) tbl[key] = function() return value end end", tbl,
					                    name, getter.value);
				} else if constexpr (std::is_same_v<std::decay_t<G>, bytecode_property>) {
				...
				detail::run_through(tbl.state(), "return function(tbl, key, x) tbl[key] = function() error(x, 2) end end", tbl, name,
				                    "getting write-only property");
			}
		}
		inline void write_setter(stack_table& tbl) const {
			if constexpr (std::is_same_v<std::decay_t<S>, bytecode_property>) {
				detail::push_const_code(tbl.state(), setter.code);
				lua_setfield(tbl.state(), tbl.slot(), name);
			} else if constexpr (!std::is_same_v<std::decay_t<S>, nil_t>) {
				tbl[name] = setter;
			} else {
				detail::run_through(tbl.state(), "return function(tbl, key, x) tbl[key] = function() error(x, 2) end end", tbl, name,
	...
	template<typename G, typename S> member_descriptor(const char*, G&&, S&&) -> member_descriptor<std::decay_t<G>, std::decay_t<S>>;

	template<typename T> static constexpr auto static_member(const char* name, T&& value) {
		return member_descriptor{name, constant_getter{std::forward<T>(value)}, nil};
	}
	template<auto Field> static constexpr auto member(const char* name) {
		if constexpr (detail::is_member_function_v<decltype(Field)>) {
			return member_descriptor{name, constant_getter{constant<Field>()}, nil};
			...
			return member_descriptor{
				name, [](lua_State*, T* p) -> decltype(auto) { return p->*Field; },
	...
	template<auto Field> static constexpr auto member(const char* name, readonly_t) {
		using T = detail::member_field_class_t<Field>;
		return member_descriptor{name, [](lua_State*, T* p) -> decltype(auto) { return p->*Field; }, nil};
	}
	template<typename G> static constexpr auto property(const char* name, G&& getter) {
		return member_descriptor{name, std::forward<G>(getter), nil};
	}
	template<typename G, typename S> static constexpr auto property(const char* name, G&& getter, S&& setter) {
		return member_descriptor{name, std::forward<G>(getter), std::forward<S>(setter)};
	}

	// Define the auto generated userdata metatable.
	//
	template<typename T> struct userdata_metatable {
		// Implement a metatable finding helper.
		//
		template<meta M, typename F> static constexpr bool find_meta(F&& fn) {
			return detail::find_tuple_if(userdata_meta<T>, [&](const auto& value) {
				if (value.field != M) return false;
				fn(value.value);
				return true;
			});
		}
		template<meta M> static constexpr bool has_meta() {
			return find_meta<M>([&](const auto&) {});
		}
		template<meta M> static inline bool set_meta(stack_table& tbl) {
			return find_meta<M>([&](const auto& value) { tbl[M] = value; });
		}
		...
		// Comparison helper. Extracts a wrapper pointer from a stack object
		// for both regular userdata and FFI cdata without throwing when the
		// wrapper has been retired/expired (since metamethods are called from
		// Lua and must produce a defined boolean result, not propagate an
		// error). Returns nullptr if the slot does not hold a userdata or
		// cdata of this type.
		//
		static userdata_wrapper<const T>* try_extract(const stack_object& obj) {
			if (obj.is<userdata_value>()) {
				auto* w = std::launder((userdata_wrapper<const T>*)obj.as<userdata_value>().pointer);
				return (w && w->check_type()) ? w : nullptr;
			}
			if (obj.is<userdata_wrapper<const T>>()) return &obj.as<userdata_wrapper<const T>>();
			return nullptr;
		}

		// Comparison of the object.
		//
		static bool eq(const stack_object& _a, const stack_object& _b) {
			auto* w1 = try_extract(_a);
			auto* w2 = try_extract(_b);
			if (!w1 || !w2) return false;
			if (w1 == w2) return true;
			if (w1->get() == w2->get()) return true;

			if constexpr (detail::EqComparable<const T>) {
				if (!w1->check_life() || !w2->check_life()) return false;
				return w1->value() == w2->value();
			} else {
				return false;
			}
		}
		static bool lt(const stack_object& _a, const stack_object& _b) {
			auto* w1 = try_extract(_a);
			auto* w2 = try_extract(_b);
			if (!w1 || !w2) return false;
			if (w1 == w2) return false;

			bool w1ok = w1->check_life();
			bool w2ok = w2->check_life();
			if (w1ok != w2ok) return w1ok < w2ok;
			if (w1->get() == w2->get()) return false;
		...
		static bool le(const stack_object& _a, const stack_object& _b) {
			auto* w1 = try_extract(_a);
			auto* w2 = try_extract(_b);
			if (!w1 || !w2) return false;
			if (w1 == w2) return true;
		...
		static void gc(lua_State* L, userdata_value u) {
			...
			if (!wrapper || !wrapper->check_type() || !wrapper->check_life()) return;
			wrapper->destroy();
		}
		...
		ULUA_COLD static void setup(lua_State* L, stack::slot i) {
			...
			if (!set_meta<meta::index>(metatable)) {
				if constexpr (detail::Indexable<T> || detail::FindIndexable<T>) metatable[meta::index] = constant<&index>();
			}
			if (!set_meta<meta::newindex>(metatable)) {
				if constexpr (detail::NewIndexable<T>) metatable[meta::newindex] = constant<&newindex>();
			}
...
return function(table)
	...
	do
		...
		elseif dindex then
			...
			if (!set_meta<meta::gc>(metatable) && !std::is_trivially_destructible_v<T>) metatable[meta::gc] = constant<&gc>();
			...
			if constexpr (has_meta<meta::pairs>()) {
				...
				//
					...
					auto iterator_triplet = [](const T* a) {
						return std::make_tuple(
					...
					metatable[meta::ipairs] = iterator_triplet;
					metatable[meta::pairs] = iterator_triplet;
				}
			}
			...
			if constexpr (has_meta<meta::add>()) {
				...
				metatable[meta::add] = [](const stack_object& a, const stack_object& b) -> decltype(auto) {
					...
					if constexpr (detail::Addable<T, T>)
						if (a_uv && b_uv) return a.as<userdata_wrapper<const T>>().value() + b.as<userdata_wrapper<const T>>().value();
					if constexpr (detail::Addable<T, double>)
						if (a_uv && b_fp) return a.as<userdata_wrapper<const T>>().value() + b.as<double>();
					if constexpr (detail::Addable<double, T>)
						if (a_fp && b_uv) return a.as<double>() + b.as<userdata_wrapper<const T>>().value();
			...
			if constexpr (has_meta<meta::sub>()) {
				...
				metatable[meta::sub] = [](const userdata_wrapper<const T>& a, const stack_object& obj) -> decltype(auto) {
					if constexpr (detail::Subable<T, T>)
						if (obj.is<userdata_wrapper<const T>>()) return a.value() - obj.as<userdata_wrapper<const T>>().value();
					if constexpr (detail::Subable<T, double>)
						if (obj.is<double>()) return a.value() - obj.as<double>();
			...
			if constexpr (has_meta<meta::idiv>()) {
				...
				metatable[meta::idiv] = [](const userdata_wrapper<const T>& a, const stack_object& obj) -> size_t {
					...
					if constexpr (detail::Idivable<T, int64_t>)
						if (obj.is<double>()) return a.value() / obj.as<int64_t>();
			...
			if constexpr (has_meta<meta::pow>()) {
				...
				metatable[meta::pow] = [](const userdata_wrapper<const T>& a, const stack_object& obj) -> decltype(auto) {
					if constexpr (detail::Powable<T, T>)
						if (obj.is<userdata_wrapper<const T>>()) return pow(a.value(), obj.as<userdata_wrapper<const T>>().value());
					if constexpr (detail::Powable<T, double>)
						if (obj.is<double>()) return pow(a.value(), obj.as<double>());
		...
		// Gets the metatable.
		//
		template<typename R = table> inline static R get(lua_State* L) {
			push(L);
